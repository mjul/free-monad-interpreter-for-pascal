...
fn il_identifier_list_from(pair: &Pair<Rule>) -> Result<il::IdentifierList, ConversionError> {
    match &pair.as_rule() {
        Rule::identifier_list => {
            ...
            let idv = il::NonEmptyVec::new(ids.to_vec()).map_err(|_| ConversionError::ConversionError(format!("Failed to create NonEmptyVec from identifier_list: {:?}", &pair)))?;
...
fn il_subprogram_declarations_from(pair: &Pair<Rule>) -> Result<il::SubprogramDeclarations, ConversionError> {
    match &pair.as_rule() {
    ...
    match &pair.as_rule() {
        Rule::compound_statement => {
            ...
            match &inners[..] {
                ...
                _ => Err(ConversionError::ConversionError("Unexpected number of pairs under compound_statement rule".to_string())),
            }
        }
        _ => Err(ConversionError::UnexpectedRuleInPair(pair.as_rule())),
    }
 ...
 */
fn il_statement_list_from_optional_statements(pair: &Pair<Rule>) -> Result<Vec<il::Statement>, ConversionError> {
    match &pair.as_rule() {
        Rule::optional_statements => {
            let inners: Vec<Pair<Rule>> = pair.clone().into_inner().into_iter().collect();
            match &inners[..] {
                [statement_list] => {
                    il_statement_list_from(statement_list)
                }
                [] => Ok(vec![]),
                _ => Err(ConversionError::ConversionError("Unexpected number of pairs under optional_statements rule".to_string())),
            }
        }
        _ => Err(ConversionError::UnexpectedRuleInPair(pair.as_rule()))
    }
...
fn il_statement_from(pair: &Pair<Rule>) -> Result<il::Statement, ConversionError> {
    match pair.as_rule() {
        Rule::statement => {
            let inners: Vec<Pair<Rule>> = pair.clone().into_inner().into_iter().collect();
            match inners.first() {
                None => Err(ConversionError::ConversionError("Unexpected empty statement".to_string())),
                Some(first_inner) => {
...
fn il_procedure_statement_from(pair: &Pair<Rule>) -> Result<il::ProcedureStatement, ConversionError> {
    match &pair.as_rule() {
        Rule::procedure_statement => {
            let inners: Vec<Pair<Rule>> = pair.clone().into_inner().into_iter().collect();
            match &inners[..] {
                ...
                _ => Err(ConversionError::ConversionError("Unexpected number of pairs under procedure_statement rule".to_string())),
            }
        }
        _ => Err(ConversionError::UnexpectedRuleInPair(pair.as_rule()))
    }
...
fn il_expression_list_from(pair: &Pair<Rule>) -> Result<il::ExpressionList, ConversionError> {
    match &pair.as_rule() {
        Rule::expression_list => {
            let exprs = &pair.clone().into_inner().into_iter()
                .filter_map(|p| {
                    match p.as_rule() {
            ...
            let ne_exprs = il::NonEmptyVec::new(exprs.to_vec())
                .map_err(|_| ConversionError::ConversionError("Failed to create NonEmptyVec from expression_list".to_string()))?;
...
fn il_expression_from(pair: &Pair<Rule>) -> Result<il::Expression, ConversionError> {
    match &pair.as_rule() {
        Rule::expression => {
            let inners: Vec<Pair<Rule>> = pair.clone().into_inner().into_iter().collect();
            match inners.len() {
                0 => Err(ConversionError::ConversionError("Unexpected empty expression".to_string())),
                1 => {
...
fn il_simple_expression_from(pair: &Pair<Rule>) -> Result<il::SimpleExpression, ConversionError> {
    match &pair.as_rule() {
        Rule::simple_expression => {
            ...
            let first_inner = inners.next()
                .ok_or(ConversionError::ConversionError("Unexpected empty simple_expression".to_string()))?;
            match &first_inner.as_rule() {
                Rule::term => {
                    // term
                ...
                Rule::sign => {
                    ...
                    let next2 =
                        inners.next()
                            .ok_or(ConversionError::ConversionError("Missing term after sign".to_string()))?;
...
fn il_term_from(pair: &Pair<Rule>) -> Result<il::Term, ConversionError> {
    match &pair.as_rule() {
        Rule::term => {
            let inners: Vec<Pair<Rule>> = pair.clone().into_inner().into_iter().collect();
            match inners.len() {
                0 => Err(ConversionError::ConversionError("Unexpected empty term".to_string())),
                1 => {
...
fn il_factor_from(pair: &Pair<Rule>) -> Result<il::Factor, ConversionError> {
    match &pair.as_rule() {
        Rule::factor => {
            let inners = pair.clone().into_inner().into_iter().collect::<Vec<Pair<Rule>>>();
            match &inners[..] {
                [p] => {
                    match p.as_rule() {
                        Rule::IDENT => {
    ...
    match &pair.as_rule() {
        Rule::sign => {
            match pair.as_str() {
                "+" => Ok(il::Sign::Plus),
                "-" => Ok(il::Sign::Minus),
                _ => Err(ConversionError::ConversionError(format!("Unexpected sign: {}", pair.as_str()))),
            }
        }
        _ => Err(ConversionError::UnexpectedRuleInPair(pair.as_rule()))
    }
        ...
        Rule::program => {
            ...
            match &inners[..] {
                [_program, id, _lparen, identifier_list, _rparen, _semicolon, declarations, subprogram_declarations, compound_statement, ..] => {
                    ...
                    let subprogram_declarations = il_subprogram_declarations_from(subprogram_declarations)?;
                ...
                _ => Err(ConversionError::ConversionError("Unexpected number of pairs under program rule".to_string())),
            }
        }
        _ => Err(ConversionError::UnexpectedRuleInPair(pair.as_rule())),
    }
...
fn parse_program_string(input: &str) -> Result<il::ProgramExpr, FrontEndError> {
    let result_pairs = PascalParser::parse(Rule::program, input)
        .map_err(|e| FrontEndError::ParseError(e))?;
    match result_pairs.len() {
        1 => {
            let pair = result_pairs.into_iter().next().expect("Expected a parse result when we know there is one");
            let result: il::ProgramExpr = il_program_from(pair).map_err(|e| FrontEndError::ConversionError(e))?;
            Ok(result)
        }
        _ => {
            Err(FrontEndError::ConversionError(ConversionError::ParseResultNotSingular))
        }
    }
}
...
mod tests {
    ...
    test_can_all!(pascal_parser_can_parse_program_declarations_and_compound_statement_empty_program_without_err, program, "program helloWorld(output); var x:integer; begin end.");
    test_can_all!(pascal_parser_can_parse_program_subprogram_declarations_and_compound_statement_empty_program_without_err, program,
        r#"
